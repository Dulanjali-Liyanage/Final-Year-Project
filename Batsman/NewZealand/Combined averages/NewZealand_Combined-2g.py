...
for t in sub:
    ...
    isempty = df.empty
    if(isempty):
        result_grams = result_grams.append({'player-1':new_list[0], 'player-2':new_list[1], 'avg':0.0}, ignore_index=True)
        continue
...
#                     count = count + df.iloc[index][player_scores[ind]]
    ...
    result_grams = result_grams.append({'player-1':new_list[0], 'player-2':new_list[1], 'avg':avg}, ignore_index=True)
...
result_grams.to_csv('F:/8TH SEM/CO425-FYP/NewZealand/Combined averages/NewZealand_2g.csv',sep=',')        
