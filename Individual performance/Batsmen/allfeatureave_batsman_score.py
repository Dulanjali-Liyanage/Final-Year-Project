# -*- coding: utf-8 -*-
"""allFeatureAve_Batsman_Score.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1KqVN_6fFMdnZATTw0Ii917APxim7a4Ir
"""
...
feature_names = ["Man of the match", "Last 4 match runs mean", "Height (cm)", "Batting Style", "NO", "HS", "HS_NO", "SR", "100", "50", "0","Mat","Inns","Runs"]
X = df[feature_names]
Y = df["Ave"]
...
#XGBoost shows better accuracy

A = np.ones([len(feature_names),len(feature_names)])
for j,p in enumerate(xgb_importance):
  for i,v in enumerate(xgb_importance):
...
df['Batsmen_score_allFeatureAve'] = df['Man of the match']*thisdict['Man of the match']+df['Last 4 match runs mean']*thisdict['Last 4 match runs mean']+df['Height (cm)']*thisdict['Height (cm)']+df['Batting Style']*thisdict['Batting Style']+df['NO']*thisdict['NO']+df['HS']*thisdict['HS']+df['HS_NO']*thisdict['HS_NO']+df['SR']*thisdict['SR']+df['100']*thisdict['100']+df['50']*thisdict['50']+df['Mat']*thisdict['Mat']+df['Inns']*thisdict['Inns']+df['Runs']*thisdict['Runs']-df['0']*thisdict['0']

df.head()

df_new = df[['player', 'Batsmen_score_allFeatureAve']]

df_new.head(20)

from google.colab import files
df_new.to_csv('allFeatureAve_batsman_score.csv' , index=False)
files.download('allFeatureAve_batsman_score.csv')
